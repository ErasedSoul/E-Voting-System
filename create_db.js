...
const db = mysql.createConnection({
    ...
    password: '',
    database: 'proj_db'    
    //-----> UN-comment this after executing '/createdb'
});
...
app.get('/fortables', (req,res) => {
    let q1 = 'CREATE TABLE voters (userid VARCHAR(50) PRIMARY KEY, name VARCHAR(50) NOT NULL, password VARCHAR(50) NOT NULL)';
    db.query(q1, (err,res) => {
        if(err) throw err;
    ...
    db.query(q10, (err,res) => {
        if(err) throw err;
        else console.log("temp table created!");
    });
})
